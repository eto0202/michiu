...
| 7      | `states`    | `st_`    | ドラッグ、リサイズ、スクロール状態                  |
| 8      | `dnd`       | `dnd_`   | ドラッグ状態                                        |
| 9      | `resize`    | `res_`   | リサイズ状態                                        |
| 10     | `scroll`    | `sc_`    | スクロール状態、スクロール量                        |
| 11     | `edit`      | `edit_`  | 選択、キャレット状態                                |
| 12     | `layouts`   | `lay_`   | 解決済みレイアウトスタイル (Taffy連携、paddingなど) |
| 13     | `scrollbar` | `bar_`   | スクロールバーレイアウト                            |
| 14     | `renders`   | `rnd_`   | ビジュアルスタイル、アニメーション・トランジション  |
| 15     | `outputs`   | `out_`   | 最終計算完了後の物理絶対座標キャッシュ              |
| 16     | `debug`     | `dbg_`   | トレース取得                                        |
