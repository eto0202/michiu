...
impl CssSignalExt for ReadSignal<CssMap> {
    #[inline]
    fn class(&self, name: &str) -> ThisStyle {
        self.get().class(name)
    }
...
impl CssSetSignalExt for ReadSignal<CssMapSet> {
    #[inline]
    fn class(&self, sheet_key: &str, class_name: &str) -> ThisStyle {
        self.get()
            .get(sheet_key)
            .map(|css_map| css_map.class(class_name))
            .unwrap_or_default()
    }

    #[inline]
    fn sheet(&self, sheet_key: &str) -> CssMap {
        self.get().sheet(sheet_key)
    }
}
    ...
    for decl in declarations {
        match decl {
            ...
            Property::FlexWrap(fw, _) => {
                ...
                style = match fw {
                    CssFw::Wrap => map_style_prop(style, target, ThisStyle::flex_wrap),
                    CssFw::WrapReverse => {
                        map_style_prop(style, target, ThisStyle::flex_wrap_reverse)
                    }
                    CssFw::NoWrap => map_style_prop(style, target, ThisStyle::flex_nowrap),
                };
            }
            ...
            Property::AlignItems(a, _) => {
                use lightningcss::properties::align::{AlignItems, OverflowPosition, SelfPosition};
                let align = match a {
                    AlignItems::BaselinePosition(_) => Some(crate::AlignItems::Baseline),
            ...
            Property::AlignSelf(a, _) => {
                use lightningcss::properties::align::{AlignSelf, OverflowPosition, SelfPosition};
                let align = match a {
                    AlignSelf::BaselinePosition(_) => Some(crate::AlignSelf::Baseline),
            ...
            Property::AlignContent(a, _) => {
                ...
                let align = match a {
                    AlignContent::Normal | AlignContent::BaselinePosition(_) => {
                        Some(crate::AlignContent::Stretch)
                    }
