...
pub(crate) unsafe fn create_shared_d3d11_texture(
    d3d11_device: &ID3D11Device,
    width: u32,
    height: u32,
) -> Result<(ID3D11Texture2D, HANDLE), Box<dyn std::error::Error>> {
    unsafe {
        // 1. 共有フラグ（MISC_SHARED および MISC_SHARED_NTHANDLE）を設定
        ...
        d3d11_device.CreateTexture2D(&desc, None, Some(&mut texture))?;
        let texture = texture.ok_or("Failed to create shared D3D11 texture")?;

        // 2. IDXGIResource1 をクエリして、NT共有ハンドルをエクスポートする
